...
class PreparerPoolpsCsv():
    ...
    # data load func
    def load_dataset(self, data: str | pd.DataFrame) -> None:
        if isinstance(data, str):
            self.data = get_data(data)
        elif isinstance(data, pd.DataFrame):
            self.data = data
        else:
            raise ValueError("Input data must be a string, a pandas DataFrame")
        # Проверяем, что данные успешно загружены
        if self.data is None or self.data.empty:
            raise ValueError("Failed to load data. The dataset is empty.")
        
    # def load_dataset(self, data: str | pd.DataFrame | BytesIO) -> None:
    #     if isinstance(data, pd.DataFrame):
    #         self.data = data  # Прямая работа с DataFrame
    #     elif isinstance(data, (str, BytesIO)):
    #         self.data = get_data(data)  # Универсальная загрузка
        # else:
        #     raise ValueError("Input data must be a string, a pandas DataFrame, or BytesIO.")
